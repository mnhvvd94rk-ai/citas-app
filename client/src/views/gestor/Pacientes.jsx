...
function Foto({ cliente, className }) {
  ...
  return <div className={`flex items-center justify-center bg-navy-700 font-bold text-green-400 ${className}`}>{ini || '·'}</div>
}
    ...
    <li className="w-full overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-navy-100">
      ...
      <button onClick={() => setAbierto((v) => !v)} className="flex w-full items-stretch gap-4 text-left transition hover:bg-navy-50/40">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center py-3">
          ...
          <span className={`mt-2 inline-block w-fit rounded-full px-2 py-0.5 text-xs font-semibold ${cliente.estado === 'NUEVO' ? 'bg-green-100 text-green-600' : 'bg-navy-100 text-navy-700'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                {editandoEdad ? (
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setInputEdad(edad ?? ''); setEditandoEdad(true) }} className="text-xs font-semibold text-navy-500 hover:text-green-600">{t('clients.edit')}</button>
                  </span>
                )}
              </div>
              <div>
