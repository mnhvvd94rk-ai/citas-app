...
function Feature({ icon, title, desc }) {
  return (
    <div className="group rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-1 hover:shadow-md">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-50 text-2xl transition group-hover:bg-brand-100">{icon}</div>
      <h3 className="mt-4 font-bold text-navy-800 transition group-hover:text-brand-600">{title}</h3>
...
export default function LandingPage() {
  ...
  const iconCls = 'h-7 w-7 text-brand-600'
  const clientes = [
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      {/* NAVBAR */}
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/90 backdrop-blur pt-[env(safe-area-inset-top)]">
        <nav className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-3 sm:gap-4 sm:px-6">
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
            ...
            <button onClick={() => navigate('/registro-cliente')} className="whitespace-nowrap rounded-lg bg-brand-500 px-2.5 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-brand-600 sm:px-4 sm:py-2 sm:text-sm">
              <span className="sm:hidden">{t('landingPublic.startFreeShort')}</span>
              <span className="hidden sm:inline">{t('landingPublic.startFree')}</span>
            </button>
          </div>
        </nav>
      </header>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-navy-800 via-navy-800 to-navy-900 text-white">
        {/* Glows de marca en las esquinas (dan profundidad al navy plano) */}
        <div className="pointer-events-none absolute -right-20 -top-16 h-72 w-72 rounded-full bg-brand-500/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-24 -left-16 h-72 w-72 rounded-full bg-brand-400/10 blur-3xl" />
        {/* Vignette radial rojo-quemado muy sutil (#B5451F) */}
        <div
          className="pointer-events-none absolute inset-0"
          style={{ backgroundImage: 'radial-gradient(60% 60% at 50% 0%, rgba(181,69,31,0.10), transparent 70%)' }}
        />
          ...
          <ol className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {pasos.map((s, i) => (
              <li key={s.n} className="relative rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-navy-700 font-bold text-brand-300">{s.n}</div>
                <h3 className="mt-3 font-bold text-navy-800">{s.title}</h3>
                <p className="mt-1 text-sm text-slate-500">{s.desc}</p>
                {i < pasos.length - 1 && <span className="absolute -right-3 top-1/2 hidden -translate-y-1/2 text-2xl text-brand-400 lg:block">→</span>}
              </li>
            ))}
          </ol>
        </Reveal>
